...
public class ImmersiveResearchWindowDrawingUtility
{
    ...
    {
        // setup table layout
        var rowHeight = (float)images[0].UIIcon.height / 3;
        var columnWidth = (float)images[0].UIIcon.width / 3;
    ...
    private void DrawListWorker(Rect refRect, string listTitle, bool hasImage)
    {
        ...
        if (hasImage)
        {
            ...
            {
                var mouseClickEvent = MouseClickCallBackEvent;
                var mouseOverEvent = MouseOverCallBackEvent;
